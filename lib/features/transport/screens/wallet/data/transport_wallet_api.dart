...
/// Transport role wallet — `doc/details.md` (`/api/transport/wallet/...`).
class TransportWalletApi {
  TransportWalletApi._();
  static final TransportWalletApi instance = TransportWalletApi._();

  Future<BuyerWalletOverview> fetchWallet() async {
    final headers = await _transportWalletHeaders();
    final uri = Uri.parse(TransportAPIController.transportWallet);
  ...
  Future<BuyerWalletPage<BuyerWalletTransaction>> fetchTransactions({
    ...
    final headers = await _transportWalletHeaders();
    final uri = Uri.parse(
      TransportAPIController.transportWalletTransactions(
        page: page,
        fromDate: fromDate,
        toDate: toDate,
        type: type,
  ...
  Future<BuyerWalletPage<BuyerPayoutRequest>> fetchPayouts({
    ...
    final headers = await _transportWalletHeaders();
    final uri = Uri.parse(
      TransportAPIController.transportWalletPayouts(page: page, status: status),
    );
    final res = await http.get(uri, headers: headers);
    _throwIfBad(res);
    final top = _decodeObj(res.body);
  ...
  Future<void> requestTopup({
    ...
    final headers = await _transportWalletHeaders();
    final uri = Uri.parse(TransportAPIController.transportWalletTopup);
  ...
  Future<void> requestPayout({
    ...
    final headers = await _transportWalletHeaders();
    final uri = Uri.parse(TransportAPIController.transportWalletPayout);
    final amountTrim = amount.trim();
